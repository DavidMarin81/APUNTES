...
- Paso 1: Configurar servicios en `Program.cs` o `Startup.cs`
    ...
    builder.Services.AddSession(options =>
    {
        ...
        options.Cookie.Name = "MiCookieSesion"; // Para dar un nombre a la cookie
    });
...
## Opciones principales de sesión (`SessionOptions`)
- `IdleTimeout`
    - Tiempo de inactividad tras el cual la sesión expira
    - Ejemplo: `options.IdleTimeout = TimeSpan.FromMinutes(30);`
- `Cookie.HttpOnly`
    - Evita que JavaScript acceda a la cookie
    - Ejemplo: `options.Cookie.HttpOnly = true;`
- `Cookie.IsEssential`
    - Marca la cookie como esencial para cumplir con GDPR y consentimiento de cookies
    - Ejemplo: `options.Cookie.IsEssential = true;`
- `Cookie.Name`
    - Cambia el nombre de la cookie de sesión
    - Ejemplo: `options.Cookie.Name = "MiCookieSesion";`
- `Cookie.SecurePolicy`
    - Define si la cookie solo se envía vía HTTPS
    - Valores posibles:
        - `CookieSecurePolicy.Always` → siempre HTTPS
        - `CookieSecurePolicy.SameAsRequest` → igual que la petición
        - `CookieSecurePolicy.None` → sin restricción
    - Ejemplo: `options.Cookie.SecurePolicy = CookieSecurePolicy.Always;`
- `Cookie.Path`
    - Ruta del cookie. Por defecto /.
    - Ejemplo: `options.Cookie.Path = "/miApp";`
- `Cookie.Domain`
    - Dominio de la cookie. Por defecto, el dominio de la aplicación.
    - Ejemplo: `options.Cookie.Domain = "midominio.com";`
- `Cookie.MaxAge`
    - Duración máxima de la cookie (opcional si usas IdleTimeout).
    - Ejemplo: `options.Cookie.MaxAge = TimeSpan.FromHours(1);`
- `IOTimeout` (menos usado)
    - Tiempo máximo para operaciones de lectura/escritura de sesión.
...
- Ejemplo:
    ...
    builder.Services.AddSession(options =>
    {
        ...
        options.Cookie.HttpOnly = true;
        options.Cookie.IsEssential = true;
        options.Cookie.Name = "MiCookieSesion";
